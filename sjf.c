...
/*
 * RANA Pvt. Ltd. — SJF Payment Queue Scheduler
...
int main() {
    ...
    Request req[MAX] = {
        {1,  "Homepage Render",    0,  8, 0,0,0,0,0,0},
        {2,  "Product Search",     1,  4, 0,0,0,0,0,0},
        {3,  "Cart Update",        2,  5, 0,0,0,0,0,0},
        {4,  "Checkout Handler",   2, 10, 0,0,0,0,0,0},
        {5,  "Payment Gateway",    3,  3, 0,0,0,0,0,0},
        {6,  "Order Confirmation", 4,  6, 0,0,0,0,0,0},
        {7,  "Inventory Check",    5,  2, 0,0,0,0,0,0},
        {8,  "Order DB Commit",    6,  7, 0,0,0,0,0,0},
        {9,  "Coupon Validation",  7,  4, 0,0,0,0,0,0},
        {10, "Session Cleanup",    8,  3, 0,0,0,0,0,0}
    };

    int n = MAX;

    printf("RANA Pvt. Ltd. SJF Payment Queue Scheduler (Windows)\n");
    ...
    while (completed < n) {
        ...
        for (int i = 0; i < n; i++) {
            if (!req[i].done && req[i].arrive <= time) {
                if (req[i].burst < min_burst) {
                    min_burst = req[i].burst;
                    idx = i;
                }
            }
        }
